...
import type { MigrationAsset, MigrationAssetDescription, MigrationReference } from "../../core/models/migration.models.js";
	...
	const editAssets = async (assetsToEdit: readonly AssetToEdit[]): Promise<readonly EditedAsset[]> => {
		...
		return await processItemsAsync<AssetToEdit, Readonly<AssetModels.Asset>>({
			...
			itemInfo: (input) => {
				return {
					itemType: "asset",
					title: input.migrationAsset.title ?? input.migrationAsset.filename,
				};
			},
