import type { AssetStateInTargetEnvironmentByCodename, ItemStateInTargetEnvironmentByCodename } from "../../core/models/core.models.js";
...
const rteRegexes = {
	elements: {
		objectTagRegex: /<object(.+?)<\/object>/gs,
		imgTagRegex: /<img(.+?)<\/img>/gs,
		figureTagRegex: /<figure(.+?)<\/figure>/gs,
		linkTagRegex: /<a(.+?)<\/a>/gs,
	},
	ids: {
...
export function richTextProcessor() {
	...
	const processAssetIds = (richTextHtml: string, replaceFunc?: IdReplaceFunc): ProcessIdsResult => {
		...
		const processedRte = richTextHtml.replaceAll(rteRegexes.elements.figureTagRegex, (figureTag) => {
			...
			if (assetIdMatch && assetIdMatch.length >= 2) {
	...
	const processItemCodenames = (
		...
		const processedRte = richTextHtml.replaceAll(rteRegexes.elements.objectTagRegex, (objectTag) => {
			...
			if (codenameMatch && codenameMatch.length >= 2) {
